 ...
 */
 ...
 main(int argc, char* argv[])
 {
  ...
  //Consumer application for /Movie
 for (int i = 0; i < 3; i++) {
  ...
  consumerHelper.SetPrefix("/Movie");
    ...
    consumer.Stop(Seconds(10)); // stop consumers at 19s, 18s, 17s, 16s
    ///////////////////////////////////////////////
    // install producer app on producer node p_i //
    ///////////////////////////////////////////////
  }

  //Consumer application for /Music
  ...
  for (int i = 0; i < 3; i++) {
    ...
    ApplicationContainer consumer = consumerHelper.Install(consumers[i]);
    consumer.Start(Seconds(10));     // start consumers at 0s, 1s, 2s, 3s
  ...
  ndn::L3RateTracer::InstallAll("rate-trace-scenario2.txt", Seconds(0.5));
